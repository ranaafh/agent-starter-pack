...
variable "project_name" {
  ...
  default     = "even-more--awesome-agent"
}
...
variable "telemetry_logs_filter" {
  ...
  default     = "labels.service_name=\"even-more--awesome-agent\" labels.type=\"agent_telemetry\""
}

variable "feedback_logs_filter" {
  type        = string
  description = "Log Sink filter for capturing feedback data. Captures logs where the `log_type` field is `feedback`."
  default     = "jsonPayload.log_type=\"feedback\" jsonPayload.service_name=\"even-more--awesome-agent\""
}
